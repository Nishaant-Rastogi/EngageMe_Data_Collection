jsPsych.extensions["webgazer"] = (function () {
    ...
    extension.initialize = function (params) {
        ...
        params.round_predictions =
            typeof params.round_predictions === "undefined"
                ? true
                : params.round_predictions;
        params.auto_initialize =
            typeof params.auto_initialize === "undefined"
                ? false
                : params.auto_initialize;
        params.sampling_interval =
            typeof params.sampling_interval === "undefined"
                ? 34
                : params.sampling_interval;
    ...
    extension.startSampleInterval = function (interval) {
        interval =
            typeof interval == "undefined" ? state.sampling_interval : interval;
    ...
    extension.onGazeUpdate = function (callback) {
        ...
        return function () {
            state.gazeUpdateCallbacks = state.gazeUpdateCallbacks.filter(function (
                item
            ) {
    ...
    function handleGazeDataUpdate(gazeData, elapsedTime) {
        if (gazeData !== null) {
            ...
            if (state.activeTrial) {
                // console.log(
                //   `handleUpdate: t = ${Math.round(gazeData.t)}, now = ${Math.round(
                //     performance.now()
                //   )}`
                // );
